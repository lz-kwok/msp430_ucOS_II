...
#if (PRODUCT_TYPE == Planting_Station)
...
uint8_t ScadaLeafTempHum_YS[Command_1Length]={0x0B,0x03,0x00,0x00,0x00,0x02,0xC4,0xA1};       //叶面温湿度
			...
			if(data[1]==0x03)
			{
				switch(data[0])
				{
					case 0x03:		//温湿度,光照
						...
						AppDataPointer->PlantingData.Humidity = (float)SensorCahe/10;
						...
						if(data[5]>=0xF0) //温度为负数
						{
							...
							AppDataPointer->PlantingData.Temperature = (float)sSensorCahe/10*(-1);
							sSensorCahe = (uint32_t)data[5]*256 + data[6]; //原始数据存放到发送字节中
						}
						else              //温度为正数
						{
							sSensorCahe = (uint32_t)data[5]*256 + data[6];
							AppDataPointer->PlantingData.Temperature = (float)sSensorCahe/10;
						}
						...
						AppDataPointer->PlantingData.Illumination = SensorCahe;
					...
					case 0x0A:		//CO2
						SensorCahe = (uint32_t)data[3]*256 + data[4];
						AppDataPointer->PlantingData.CO2 = SensorCahe;
					...
					case 0x0B:		//叶面湿度
						SensorCahe = (uint32_t)data[3]*256 + data[4];
						AppDataPointer->PlantingData.LeafHumidity = (float)SensorCahe/10;
...
//								/**************土壤温度**************/
//								SensorCahe = (uint32_t)data[3]*256 + data[4];
//								AppDataPointer->PlantingData.SoilTemp = (float)SensorCahe/100;
...
//								AppDataPointer->PlantingData.SoilHum = (float)SensorCahe/100;
...
//								AppDataPointer->WetherSoilData.SoilConductivity = SensorCahe;
...
//									SendNByteToUart3(ScadaSoilTempHumCond_RK,Command_1Length);  //土壤温湿度电导率_RK
			...
			case 3:
				...
				break;
			case 4:
				SendNByteToUart3(ScadaLeafTempHum_YS,Command_1Length);   //叶面湿度
				break;
			default:
				break;
		}
