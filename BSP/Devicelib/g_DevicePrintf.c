...
//*****************************************************************************
//
// Converts a string representing a decimal value to an uint32_t.
//
// Returns the uint32_t integer value.
//
...
//*****************************************************************************
static uint32_t
decstr_to_u(char *pcStr, uint32_t *pui32CharCnt)
{
...
static int ftoa(float fValue, char *pcBuf, int iPrecision)
{
    ...
    {
        // "0.0"
        *(uint32_t*)pcBuf = (uint32_t)0x00 << 24 | ((uint32_t)'0' << 16) | ((uint32_t)'.' << 8) | ((uint32_t)'0' << 0);
        return 3;
    }
...
//******************************************************************************
uint32_t
am_util_stdio_vsprintf(char *pcBuf, char *pcFmt, va_list pArgs)
{
    char *pcStr;
    uint64_t ui64Val;
    int64_t i64Val;
                ...
                if ( pcBuf )
                {
                    ...
                    *(uint32_t*)pcBuf = 20;
                    ...
                    if ( iVal < 0 )
                    {
                        ...
                        {
                            u32PrntErrVal = ((uint32_t)0x00 << 24) | ((uint32_t)'0' << 16) |
                                            ((uint32_t)'.' << 8)   | ((uint32_t)'0' << 0);  // "0.0"
                        }
                        else if ( iVal == AM_FTOA_ERR_VAL_TOO_LARGE )
                        {
                            u32PrntErrVal = ((uint32_t)0x00 << 24) | ((uint32_t)'#' << 16) |
                                            ((uint32_t)'.' << 8)   | ((uint32_t)'#' << 0);  // "#.#"
                        }
                        else
                        {
                            u32PrntErrVal = ((uint32_t)0x00 << 24) | ((uint32_t)'?' << 16) |
...
//******************************************************************************
uint32_t
am_util_stdio_sprintf(char *pcBuf, char *pcFmt, ...)
{
