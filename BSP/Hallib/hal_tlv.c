...
void Get_TLV_Info(uint8_t tag, uint8_t *length, uint16_t ** data_address)
{
  ...
  while((TLV_address < ( char *)TLV_END) && (*TLV_address != tag) && (*TLV_address != TLV_TAGEND)) // do range check first
  {
...
uint16_t Get_TLV_Memory(uint8_t instance)
{
    ...
    Get_TLV_Info(TLV_PDTAG, &bPDTAG_bytes, (uint16_t **) &pPDTAG); // Get Peripheral data pointer
