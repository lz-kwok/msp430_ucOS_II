...
#define ST_CLUST(dir,cl) {ST_WORD(dir+DIR_FstClusLO, cl); ST_WORD(dir+DIR_FstClusHI, (uint32_t)cl>>16);}
...
)
{
	...
	if ((uint16_t)*path < ' ') {			/* Nul path means the start directory itself */
		res = dir_sdi(dj, 0);
		dj->dir = 0;
...
)
{
	...
	*path = 0;
	res = chk_mounted((const char**)&path, &dj.fs, 0);	/* Get current volume */
	if (res == FR_OK) {
		INIT_BUF(dj);
...
)
{
	...
	for (cc = res = 0; cc != EOF; res += cc) {
		...
		case 'S' :					/* String */
			...
			continue;
		case 'C' :					/* Character */
			cc = f_putc((char)va_arg(arp, int), fil); continue;
		case 'B' :					/* Binary */
			r = 2; break;
		case 'O' :					/* Octal */
			r = 8; break;
